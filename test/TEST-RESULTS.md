...
## Summary: 119 PASS / 63 FAIL of 182

| Status | Count | Meaning |
|---|---|---|
| PASS | 119 | within realistic tolerance (incl. 3 HANG — see below) |
...
| NONCONVERGE | 8 | optimizer/SCF not converged (134e + 142–144 open-shell KS-gradient, see below) |
| INCOMPLETE | 0 | (none this run) |

## DFT/KS fixes applied 2026-06-02 (xintgrt group: 0/8 → 5/8 PASS, DFT now functional)
...
| `vksdint/equi.F` | gradient symmetry-equivalence map | closed-shell KS forces fully converge — **145.ksgrad PASS** |
| `intgrt/numinteff.F` | alternate XC path | same defect, fixed defensively |

`equi.F` is a sharper instance: the caller passes three sections of the **same `kscore`
array** as the dummy arguments `comp`, `comppov`, `e`
(`equi(kscore(pcompmemb),..,kscore(pcomppopv),kscore(equ))`); `-O1+` assumed the dummies
did not alias and reordered the `e(ref,..)` stores against the reads, mislabelling the
symmetry-equivalent atoms. Fixed with `volatile kscore, comp, comppov, e` (marking
`volatile kscore` alone was insufficient — the aliasing is through the dummy args).

Net: **104.ks, 105.ks, 106.ks, 145.ksgrad** now PASS (correct KS energies/geometry/
forces), joining **146.hfdft.ccsd** → **5/8**. All `-O2`-clean (no per-file `-O0`).

### (c) open-shell HFDFT gradient — blocks 142/143/144 (left open, NOT an -O2 bug)
`142/143/144.hfdft` are open-shell (`REF=UHF`, `MULTI=2`) HF-DFT (`kspot=hf`,
`func=becke,lyp`) single-gradient checks (`OPT_MAXCYC=1`, empty TEST.DAT → must converge
in one cycle). Their analytic gradient computes and is translationally invariant, but
carries a residual force (RMS ~0.017) so the one optimization step never converges. This
is **unchanged at `-O0`** (both `intgrt` and `vksdint`), so it is a genuine open-shell
KS-gradient logic/port defect, not a `-O2` miscompile — a separate, deeper item.
...
| test | detail |
|---|---|
| zmat.142.hfdft | open-shell (UHF/HFDFT) analytic gradient carries a residual force RMS ~0.017 (unchanged at -O0) so OPT_MAXCYC=1 never converges -- open-shell KS-gradient logic defect, not -O2 |
| zmat.143.hfdft | same as 142 -- open-shell KS-gradient logic defect |
| zmat.144.hfdft | same as 142 -- open-shell KS-gradient logic defect |
...
`†` = numerically correct but **HANGs** (non-terminating xvip loop; see the HANG section).
    ...
    zmat.104.ks zmat.105.ks zmat.106.ks zmat.145.ksgrad zmat.146.hfdft.ccsd
